...
function Tick(tick)
    ...
    for i,v in ipairs(heros) do
		...
		if me.alive then
			...
			if Q and Q:CanBeCasted() and (v.health > 0 and v.health < dmg[Q.level]) and GetDistance2D(v,me) <= Q.castRange and  SleepCheck("q") then
				me:CastAbility(Q,v)
				Sleep(400+client.latency,"q")
			end

			if R and R:CanBeCasted() and not buff and distance <= R.castRange and SleepCheck("r") then
